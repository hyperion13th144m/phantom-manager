...
搭載メモリに合わせて次の値を書いて保存してください。

| パソコンの搭載メモリ | `memory` | `swap` |
|---|---|---|
| 32GB | `16GB` | `8GB` |
| 16GB | `10GB` | `8GB` |
| 12GB | `7GB` | `12GB` |
...
`memory` だけを書いて `swap` を書かないと、取り込みが violet（画像を解析するサービス）まで
進んだところで**パソコンが固まったようになり、管理画面の「中止」も「停止」も効かなくなります。**

WSL2 のメモリが足りなくなったとき、swap があれば当面使わないデータをディスクへ逃がせるので、
処理は遅くなるだけで進みます。swap が無いと逃がす先が無く、空きが底をついたまま回復しません。
このとき Ubuntu と Docker Desktop をつなぐ通信路も新しく作れなくなるため、Docker を使う操作が
すべて無反応になります。管理画面は Docker 経由でサービスを止めているので、
**画面からは止められなくなります。**

そうなった場合は、管理者の PowerShell で次を実行してください。

```powershell
wsl --shutdown
```

そのあと Docker Desktop を再起動すれば元に戻ります。取り込みは最初からやり直しになりますが、
すでに登録済みのデータは読み飛ばされるので、2 回目は速く終わります。

#### 搭載 12GB のパソコンの場合

12GB は phantom にとってかなり厳しい構成です。動きますが、次の点を守ってください。

- **`swap=12GB` を必ず書く。** 12GB の環境では、swap があるかどうかで動くか固まるかが決まります。
- **`memory` を 8GB 以上にしない。** Windows 本体に残る分が 4GB を切ると、今度は Windows 側が
  遅くなり、結局は全体が重くなります。7GB にして、足りない分を swap で受けるほうが安定します。
- **swap の置き場所が SSD であること。** HDD のパソコンでは、swap を使い始めた時点で
  実用的な速度が出ません。
- **[5.1](#51-メモリの割り当てについて自動) の自動設定はそのままにする。** Elasticsearch の
  値をこれ以上下げても空きはほとんど増えず、かえって取り込みが遅くなります。
- 取り込みには時間がかかります。**約 36,000 ファイルで 12 時間ほど**が目安です。
  夜間など、パソコンを他の用途に使わない時間に実行してください。

上の条件を守れば、12GB のパソコンでも最後まで通ります。実際に Windows 10 / 搭載 12GB /
`memory=7GB` / `swap=12GB` の環境で、約 36,000 ファイル（登録された文書は 20,810 件）の
取り込みが **12 時間 5 分で、一度も止まらずに完了**しています。

このとき swap は 12GB のうち 0.5GB ほどしか使われていません。swap は「たくさん使うため」ではなく
「いざというときの逃げ場を用意するため」に必要だと考えてください。逃げ場が無い状態が、
一つ上の [swap の行を省略しないでください](#swap-の行を省略しないでください) で説明した
固まる状態です。
...
### 7.3 Elasticsearch データを削除

検索インデックスが壊れた、あるいは作り直したいときに使います。
**「Elasticsearch データを削除」** をクリックすると確認が出るので、了承すると
Elasticsearch のボリュームが削除されます。

- 消えるのは**検索インデックスだけ**です。取込先・展開先のファイルはそのまま残るので、
  次に「起動」するとインデックスは自動的に作り直されます。
- 作り直しには初回と同じだけ時間がかかります。
- 先に **「停止」でコンテナを削除しておく必要があります**。サービスが残っている間は
  ボタンが押せません（Docker が使用中のボリュームを削除できないため）。
...
- **データの置き場所が変わりました。** 旧バージョンはリポジトリの中の `internet-app-data` に
  コピーしていましたが、新バージョンは `~/phantom/src`（[5](#5-データディレクトリの設定) の取込先）です。
...
| 取り込みの途中で止まり、「中止」も「停止」も効かない | メモリ不足です。管理者の PowerShell で `wsl --shutdown` を実行し、Docker Desktop を再起動してください。再発を防ぐには [1.5](#15-wsl2-にメモリを割り当てる重要) の `swap` を設定してください |
| 「中止」を押しても何も起きない | 取り込みはコンテナの中で動いていて、管理画面の処理ではありません。止めるときは [7.2 停止](#72-停止) を使ってください |
