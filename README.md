...
`.env.docker` を初めて生成するときだけ、Elasticsearch を次の値に絞ります。
...
サンプルの `-Xms2g -Xmx2g` / `4g` は、メモリを丸ごと渡せる Linux ホスト向けです。
WSL2 は既定でホストの半分しか VM に渡さず、[INSTALL.md](INSTALL.md) では 16GB の PC で 10GB に固定しています。
そのうち 1〜1.5GB は VM 自身が使うので、サンプルのままでは violet が CLIP を読む瞬間のピーク 4GB を賄えず、
取り込みの途中で OOM kill されます。根拠は phantom-release の `docs/production.md`（「16 GB のホストでの回し方」）です。

**2 回目以降の保存では書き換えません。** WSL2 に多く割り当てて値を上げた人のものは、その人の設定です。
置き換えた元の値は直前の行にコメントとして残ります。
